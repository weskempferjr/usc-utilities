...
abstract class Usc_Utilities_Meta_Box {
	...
	public function __constructor() {
		$this->nothing_selected = __('-- Choose one --', USC_UTIL_TEXTDOMAIN );
												...
												)
	{
		...
		$ul .= '<button type="button" title="' . $this->get_tooltip( 'add_button' ) . '" class="' . $add_button_class . '" id="' . $name . ' add-button" formaction="javascript:void(0)">' . __( '+', USC_UTIL_TEXTDOMAIN ) . '</button>';
	...
	protected function null_option() {
		$option_tag = '<option value="">' .  __('--Choose One--', USC_UTIL_TEXTDOMAIN ) . '</option>';
		return $option_tag;
	}
